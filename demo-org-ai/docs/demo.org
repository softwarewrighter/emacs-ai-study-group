...
(unless (package-installed-p 'org-ai)
  ...
  (condition-case err
      (progn
        ...
        (message "✓ org-ai installed successfully"))
    (error
     (message "✗ Failed to install org-ai: %s" err))))

;; Load org-ai
(require 'org-ai nil t)

(if (featurep 'org-ai)
    (message "✓ org-ai is loaded and ready!")
  (message "✗ org-ai could not be loaded"))
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
(if (featurep 'org-ai)
    (message "✓ org-ai is loaded")
  (if (locate-library "org-ai")
      (progn
        (require 'org-ai)
        (message "✓ org-ai loaded successfully"))
    (message "✗ org-ai not found. Execute the blocks above to install it.")))
#+END_SRC
...
(with-eval-after-load 'org
  ...
  (message "✓ org-ai-mode will be enabled in all org buffers"))
#+END_SRC
...
🎨 WHAT EXPORTS
  ✓ #+begin_ai blocks (as code blocks)
...
🚨 DIALECT CONFUSION: Common Lisp vs Emacs Lisp

Most common problem when asking for code!

❌ WRONG - AI generates Common Lisp:
#+begin_ai
...
(defun print-list (items)
  (loop for item in items
    do (format t \"%%s\" item)))    ; ← Common Lisp!
#+end_ai

✓ CORRECT - Request Emacs Lisp specifically:
...
(defun print-list (items)
  (dolist (item items)
    (message \"%%s\" item)))        ; ← Emacs Lisp!
#+end_ai

Red Flags to Spot:
  🚩 format t          → Should be: message
  🚩 (loop for ...)    → Should be: dolist or cl-loop
  🚩 defparameter      → Should be: defvar
  🚩 plain incf/decf   → Should be: cl-incf/cl-decf
...
;; C-c C-c to execute
;; If error 'stringp t' → Common Lisp detected!
#+end_src"))
...
(message "✓ dotimes for numbers, dolist for lists!")
#+END_SRC
...
(defgroup my-pkg nil           ; ← Not needed!
  "Package"
  :group 'tools)

(defcustom my-opt t            ; ← Not needed!
  "Option"
  :type 'boolean)

(defun my-function ()          ; ← This is what you wanted
  (message "Hello"))

(provide 'my-pkg)              ; ← Not needed!
...
1. Learning Journal:
   ~/org/learning/
     ├── 2025-01-emacs-lisp.org
     ├── 2025-01-web-development.org
     └── topics-index.org

2. Project Documentation:
   ~/projects/my-package/docs/
     ├── design-decisions.org (with #+begin_ai)
     ├── api-reference.org
     └── troubleshooting.org

3. Research Notes:
   ~/org/research/
     ├── emacs-performance.org
     ├── ai-tools-comparison.org
     └── best-practices.org
...
❌ API Key Not Found
   ✓ Check: org-ai-openai-api-token
   ✓ Or: (getenv \"OPENAI_API_KEY\")
   ✓ Fix: (setq org-ai-openai-api-token \"sk-...\")

❌ org-ai-mode Not Enabled
   ✓ Check: M-x org-ai-mode
   ✓ Fix: (add-hook 'org-mode-hook #'org-ai-mode)
   ✓ Or: Add to config

❌ Block Doesn't Execute
   ✓ Verify: #+begin_ai (not #+BEGIN_AI)
   ✓ Cursor must be on block
   ✓ Press: C-c C-c (not C-c C-x)

❌ No Response Appears
   ✓ Check *Messages* buffer for errors
   ✓ Verify API key is valid
   ✓ Check internet connection
   ✓ Try :stream nil for debugging

❌ Wrong Model Used
   ✓ Set in block: #+begin_ai :model gpt-4o
   ✓ Or globally: org-ai-default-chat-model

❌ Images Don't Generate
   ✓ Use :image flag
   ✓ Check org-ai-image-directory exists
   ✓ Verify DALL-E access
...
❌ Slow Responses
   ✓ Use :stream t to see progress
   ✓ Try smaller model
   ✓ Reduce :max-tokens
   ✓ Check network

❌ Context Not Maintained
   ✓ Keep conversation in same block
   ✓ Don't close/reopen blocks
   ✓ Each block is separate session"))
...
📊 org-ai
  ...
  Pros:
    ✓ Reproducible research format
    ✓ Export-friendly (PDF, HTML)
    ✓ Version control friendly
    ✓ Structured documentation
    ✓ Image generation built-in
    ✓ Context preserved in document
  Cons:
    ✗ Only works in org-mode
    ✗ OpenAI only (no Anthropic/Ollama)
    ✗ Requires org knowledge
  Best for:
...
📊 gptel
  ...
  Pros:
    ✓ Works in any buffer
    ✓ Multiple backends (OpenAI, Claude, Ollama)
    ✓ Minimal setup
    ✓ Lightweight
  Cons:
    ✗ Less structure
    ✗ No native org integration
    ✗ No image generation
  Best for:
    - Quick queries
