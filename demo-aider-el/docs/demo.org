...
(unless (package-installed-p 'aider)
  ...
  (condition-case err
      (progn
        ...
        (message "✓ aider installed successfully"))
    (error
     (message "✗ aider not available on MELPA yet.\nInstall from: https://github.com/tninja/aider.el\nOr use straight.el"))))

;; Try to load aider
(require 'aider nil t)

(if (featurep 'aider)
    (message "✓ aider.el is loaded and ready!")
  (message "✗ aider.el could not be loaded. Manual installation may be required."))
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
(if (featurep 'aider)
    (message "✓ aider.el is loaded")
  (message "✗ aider.el is not loaded. Execute the blocks above to install it, or install manually from GitHub."))
#+END_SRC
...
(let ((aider-path (executable-find "aider")))
  (if aider-path
      (message "✓ Aider found at: %s" aider-path)
...
(let ((openai-key (getenv "OPENAI_API_KEY"))
  ...
  (cond
   (openai-key (message "✓ OpenAI API key is configured"))
   (anthropic-key (message "✓ Anthropic API key is configured"))
   (t (message "✗ No API key found. Set OPENAI_API_KEY or ANTHROPIC_API_KEY"))))
#+END_SRC
...
❌ PITFALL: Adding too many files
   ✓ SOLUTION: Only add relevant files, use /drop

❌ PITFALL: Vague requests
   ✓ SOLUTION: Be specific: 'Add X to Y using Z approach'

❌ PITFALL: Not reviewing changes
   ✓ SOLUTION: Always use /diff before accepting

❌ PITFALL: Working without git
   ✓ SOLUTION: Initialize repo first: git init

❌ PITFALL: Not using /undo when needed
   ✓ SOLUTION: /undo is your friend - use it!

❌ PITFALL: Forgetting to test AI changes
   ✓ SOLUTION: Always test before committing

❌ PITFALL: Using expensive models for simple tasks
