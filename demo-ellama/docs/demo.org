...
** To Capture YOUR Own Results

For live AI interactions:

1. Start Ollama: ~ollama serve~
2. Open this file in Emacs
3. Execute individual blocks: ~C-c C-c~ on each block
4. Results will replace the examples below each block
5. Export: ~C-c C-e h h~ or ~./export.sh~
...
(unless (package-installed-p 'llm)
  ...
  (condition-case err
      (progn
        (package-install 'llm)
        (message "✓ llm installed successfully"))
    (error
     (message "✗ Failed to install llm: %s" err))))
...
(unless (package-installed-p 'ellama)
  ...
  (condition-case err
      (progn
        ...
        (message "✓ ellama installed successfully"))
    (error
     (message "✗ Failed to install ellama: %s" err))))
...
(if (and (featurep 'llm) (featurep 'ellama))
    (message "✓ Ellama and dependencies are loaded and ready!")
  (message "✗ Some packages could not be loaded"))
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
(if (featurep 'ellama)
    (message "✓ Ellama is loaded")
  (if (locate-library "ellama")
      (progn
        (require 'ellama)
        (message "✓ Ellama loaded successfully"))
    (message "✗ Ellama not found. Install from MELPA.")))
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
(if (featurep 'llm)
    (message "✓ llm package is loaded")
  (if (locate-library "llm")
      (progn
        (require 'llm)
        (message "✓ llm package loaded"))
    (message "✗ llm package not found. It's required for Ellama.")))
#+END_SRC
...
(message "✓ Ellama configured with llama3.2 model")
#+END_SRC
...
Naming Strategies:
  1. Generated by LLM:
     ...
     → AI generates descriptive name

  2. Current timestamp:
     (setopt ellama-naming-scheme 'ellama-generate-name-by-time)
     → Uses timestamp: 2025-01-03-14-30-00
...
Loading Sessions:
  ...
  → Select from list of previous chats
...
1. Build on Previous Responses:
   ...
   You: 'Show me an example'  ← Uses previous context
   You: 'Now make it thread-safe' ← Continues context

2. Reference Earlier Code:
   You: 'Write a function to parse JSON'
   AI: [provides function]
   You: 'Add error handling to that function' ← Knows which one
...
✓ Saves conversation for reference"))
...
🚨 CRITICAL: Common Lisp vs Emacs Lisp Confusion
...
Red Flags in Generated Code:
  ⚠️  format t              → Should be: message
  ⚠️  (loop for ...)        → Should be: dolist, dotimes, or cl-loop
  ⚠️  defparameter          → Should be: defvar
  ⚠️  (incf x)              → Should be: (cl-incf x) or (setq x (1+ x))
  ⚠️  (decf x)              → Should be: (cl-decf x) or (setq x (1- x))
  ⚠️  remove-if without cl- → Should be: cl-remove-if or seq-filter

Real Example - FizzBuzz:
AI might generate:
  (format t \"Fizz\")        ; ← BREAKS! Wrong dialect

Should be:
  (message \"Fizz\")         ; ← WORKS! Correct for Emacs

Quick Fix Strategy:
1. Search generated code for 'format t'
2. Replace with 'message'
3. Check for 'loop' → use 'dolist' or 'cl-loop'
4. Test in *scratch* buffer before using

Better Prompts for Ellama:
  ✓ 'Write EMACS LISP (not Common Lisp) code for...'
  ✓ 'Use Emacs Lisp message function for output'
  ✓ 'Use dolist and dotimes for iteration'
  ✓ 'Follow Emacs Lisp conventions'

If you get Common Lisp code:
  → Ask: 'Rewrite this for Emacs Lisp specifically'
  → Ask: 'Replace Common Lisp functions with Emacs equivalents'"))
...
REPLACE THESE:
  (format t \"%%s\" x)      → (message \"%%s\" x)
  (loop for ...)          → (dolist ...)
  (defparameter x val)    → (defvar x val)
  (incf x)                → (cl-incf x) or (setq x (1+ x))
  (decf x)                → (cl-decf x) or (setq x (1- x))
  (remove-if pred list)   → (seq-filter (lambda (x) (not (pred x))) list)
  (remove-if-not p list)  → (seq-filter p list)
...
3. If error mentions 'stringp t' → Common Lisp detected!
...
(defun greet-wrong ()
  (let ((names '("Alice" "Bob")))
    (dotimes (name names)      ; ← WRONG! dotimes needs a NUMBER
...
(defun greet-correct ()
  (let ((names '("Alice" "Bob")))
    (dolist (name names)       ; ← RIGHT! dolist for lists
      (message "Hello, %s!" name))))

(greet-correct)

(message "✓ Remember: dotimes=numbers, dolist=lists")
#+END_SRC
...
Common Ellama Mistakes:
  ✗ (dotimes (name names) ...)     ; Wrong type!
  ✓ (dolist (name names) ...)      ; Correct

  ✗ (dolist (i 10) ...)            ; Wrong type!
  ✓ (dotimes (i 10) ...)           ; Correct"))
...
(defgroup my-group nil           ; ← Not requested!
  "Settings"
  :group 'tools)

(defcustom my-var t              ; ← Not requested!
  "A variable"
  :type 'boolean)

(defun my-function ()            ; ← What you wanted
  (message "Hello"))

(provide 'my-function)           ; ← Not requested!
...
❌ Wrong Language Response
   ✓ Set: (setopt ellama-language \"English\")
   ✓ Or specify in prompt

❌ Code Suggestions Don't Work
   ✓ Use code-specific model: codellama
   ✓ Be more specific in prompts
   ✓ Provide more context

❌ Can't Load Old Sessions
   ✓ Check: ellama-sessions-directory
   ✓ Verify files exist: ls ~/.emacs.d/ellama-sessions"))
...
💾 MEMORY USAGE (Smallest to Largest)
...
Pure Coding:
  → deepseek-coder or codellama

Quick Tasks:
  → mistral or phi

Learning:
  → llama3.2 (good explanations)

Limited RAM:
  → phi or mistral (smaller models)
...
Use Case 2: Code Documentation
  ...
  Select code → ellama-ask-selection
...
Ellama vs gptel:
  ...
  → Use both! gptel for cloud APIs, Ellama for local
...
Happy AI-assisted Emacs hacking! 🎉"))
